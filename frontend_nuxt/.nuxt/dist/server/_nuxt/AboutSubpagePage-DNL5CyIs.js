    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (unref(pendingShell) || unref(pendingPage)) {
        ...
        if (unref(pageData).content_html) {
          _push(`<div class="prose prose-slate mt-6 max-w-none prose-img:rounded-sm prose-img:border prose-img:border-slate-200 prose-img:shadow-none">${unref(pageData).content_html ?? ""}</div>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(pageData).gallery_sections?.length) {
        ...
        if (unref(pageData).video_items?.length) {
...
//# sourceMappingURL=AboutSubpagePage-DNL5CyIs.js.map
