<template>
  <div class="min-h-screen bg-[#f7f7f7] text-slate-900">
    ...
    <div v-else-if="shellError || errorProducts || !shell || !activeCategory" class="grid min-h-screen place-items-center px-6">
      <div class="max-w-lg rounded-[2rem] border border-red-200 bg-white p-10 text-center shadow-[0_25px_80px_rgba(15,23,42,0.08)]">
        <p class="text-sm font-semibold uppercase tracking-[0.22em] text-[#e83146]">Category unavailable</p>
        <h1 class="mt-4 text-3xl font-semibold text-slate-900">Unable to load this product category.</h1>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <section class="relative overflow-hidden bg-slate-950">
        <div v-if="heroImage" class="absolute inset-0">
          <img :src="heroImage" :alt="categoryContent.displayTitle" class="h-full w-full object-cover" />
          <div class="absolute inset-0 bg-[linear-gradient(90deg,rgba(2,6,23,0.9)_0%,rgba(2,6,23,0.78)_42%,rgba(2,6,23,0.38)_100%)]" />
        </div>
        <div v-else class="absolute inset-0 bg-[linear-gradient(135deg,#0b4b99_0%,#07152b_100%)]" />

        <div class="relative mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
          <div class="grid gap-8 lg:grid-cols-[minmax(0,1fr),320px] lg:items-end">
            <div class="max-w-3xl text-white">
              <p class="text-xs font-semibold uppercase tracking-[0.32em] text-white/70">{{ categoryContent.keywords }}</p>
              <h1 class="mt-4 max-w-3xl text-4xl font-semibold leading-tight sm:text-5xl">{{ categoryContent.displayTitle }}</h1>
              <p class="mt-5 max-w-2xl text-base leading-8 text-white/80 sm:text-lg">{{ categoryContent.description }}</p>
              <p class="mt-4 max-w-2xl text-sm leading-7 text-white/70 sm:text-base">{{ categoryContent.supportText }}</p>

              <div class="mt-6 flex flex-wrap gap-3">
                <span
                  v-for="highlight in categoryContent.highlights"
                  :key="highlight"
                  class="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/85 backdrop-blur"
                >
                  {{ highlight }}
                </span>
              </div>
            ...
            <div class="rounded-[1.75rem] border border-white/15 bg-white/10 p-6 text-white shadow-[0_20px_60px_rgba(2,6,23,0.28)] backdrop-blur">
              <p class="text-xs font-semibold uppercase tracking-[0.28em] text-white/70">In this collection</p>
              <ul class="mt-5 space-y-3">
                <li
                  v-for="product in featuredProducts"
                  :key="product.id"
                  class="rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-sm leading-6 text-white/90"
                >
                  {{ product.name }}
                </li>
              </ul>
              <p v-if="featuredProducts.length === 0" class="mt-5 text-sm leading-7 text-white/70">
                Product details for this category are loading into the migrated catalog and will appear here as inventory is updated.
              </p>
              <NuxtLink
                to="/contact"
        ...
        <div class="grid gap-8 lg:grid-cols-[280px,1fr]">
          ...
          <section>
            <div class="rounded-sm border border-slate-200 bg-white p-6 text-sm leading-8 text-slate-600 shadow-sm lg:p-8">
              <h2 class="text-2xl font-semibold text-slate-900">{{ activeCategory.name }} Solutions</h2>
              <p class="mt-4">
                {{ categoryContent.description }}
              </p>
              <p class="mt-4">
                {{ categoryContent.supportText }}
              </p>
              <div class="mt-6 flex flex-wrap gap-3">
                <span
                  v-for="highlight in categoryContent.highlights"
            ...
            <div class="mt-6 flex items-center justify-between border-b border-slate-200 pb-3">
              <h1 class="text-2xl font-semibold text-slate-900">{{ activeCategory.name }}</h1>
